...
export function exportSequenceDiagram(diagram, { sourceLabel, primitives }) {
  ...
  if (diagram.title) {
    elements.push(text({
      x: 20, y: 16, width: diagram.width - 40, height: 30,
      value: diagram.title, fontSize: FONT.sizePlaneTitle,
      color: HEAD_STROKE, align: "center",
    }));
  }
  ...
  for (const p of diagram.participants) {
    ...
    if (p.shape !== "actor") {
      ...
      }, elements, { rect, text, ellipse, line });
    }
  }
  ...
  for (const n of diagram.notes) {
    ...
    elements.push(text({
      ...
      value: n.text, fontSize: FONT.sizeDescription,
      color: "#000",
    }));
  }
...
function renderParticipantHead(p, elements, { rect, text }) {
  ...
  elements.push(rect({
    ...
    strokeColor: HEAD_STROKE, backgroundColor: HEAD_FILL,
  }));
  if (p.stereotype) {
    elements.push(text({
      x, y: p.headY + 4,
      width: p.headWidth, height: FONT.sizeDescription * FONT.lineHeight,
      value: `«${p.stereotype}»`,
  ...
  elements.push(text({
    x, y: p.headY + (p.headHeight - th) / 2 + (p.stereotype ? 6 : 0),
    width: p.headWidth, height: th,
    value: p.title, fontSize: FONT.sizeTitle,
    color: HEAD_STROKE, align: "center",
  }));
}
...
function renderActorHead(p, elements, { line, ellipse, text }) {
  ...
  elements.push(ellipse({
    ...
    strokeColor: ACTOR_STROKE, backgroundColor: "#ffffff",
  }));
  elements.push(line({
  ...
  elements.push(line({
    points: [{ x: cx - 12, y: bodyTop + 6 }, { x: cx + 12, y: bodyTop + 6 }],
    strokeColor: ACTOR_STROKE,
  }));
  elements.push(line({
    points: [{ x: cx, y: bodyBottom }, { x: cx - 10, y: bodyBottom + 12 }],
    strokeColor: ACTOR_STROKE,
  }));
  elements.push(line({
    points: [{ x: cx, y: bodyBottom }, { x: cx + 10, y: bodyBottom + 12 }],
  ...
  elements.push(text({
    x: cx - p.headWidth / 2, y: labelY,
    width: p.headWidth, height: FONT.sizeTitle * FONT.lineHeight,
    value: p.title, fontSize: FONT.sizeTitle,
    color: ACTOR_STROKE, align: "center",
  }));
}
...
function renderMessage(m, elements, { arrow, text }) {
  ...
  if (m.label) {
    ...
    elements.push(text({
      x: x1, y: m.y - FONT.sizeDescription * FONT.lineHeight - 2,
      width: x2 - x1, height: FONT.sizeDescription * FONT.lineHeight,
      value: m.label, fontSize: FONT.sizeDescription,
...
function renderSelfMessage(m, elements, { arrow, text }) {
  ...
  if (m.label) {
    elements.push(text({
      x: x + off + 6, y: m.y + 4,
      width: 200, height: FONT.sizeDescription * FONT.lineHeight,
      value: m.label, fontSize: FONT.sizeDescription, color: "#222",
    }));
  }
}
