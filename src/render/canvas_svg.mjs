...
export function excalidrawJsonToCanvasSvg(doc, opts = {}) {
  ...
  // markers). After we strip the <svg> wrapper that <defs> would otherwise end
  // up inside the canvas <g transform>, which prevents Safari and resvg from
  // resolving marker IDs and font-face rules reliably. Hoist it to the canvas
  // root so it stays a direct child of the outer <svg>.
