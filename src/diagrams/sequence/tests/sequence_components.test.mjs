...
import { SEQUENCE_COMPONENT_EXAMPLES } from "../docs/coverage_examples.mjs";
import { SEQUENCE_SPACING, arrowLabelBudget } from "../spacing.mjs";
import { FONT } from "../../../general/style/text.mjs";
import { writeSequenceOutput } from "./output.mjs";
...
test("sequence component examples parse, render, and write review artifacts", async () => {
  for (const example of SEQUENCE_COMPONENT_EXAMPLES) {
    ...
    writeSequenceOutput(`${example.id}.puml`, example.source);
    writeSequenceOutput(`${example.id}.excalidraw.json`, JSON.stringify(doc, null, 2));
    writeSequenceOutput(`${example.id}.svg`, svg);
  }
});
